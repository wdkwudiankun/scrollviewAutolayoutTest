...
- (void)viewDidLoad {
  ...
  tipLabel.text = @"这个标签是使用Masonry完成的纯代码自动布局。这个标签的约束添加方式为：使左、上与父视图的左、上分别相等，使右边与self.view的右边的相距-10，就可以确定其宽。这里不能使用使右等于scrollview的右，因为scrollview是可以滚动的，其右是不确定的。但是可以使用scrollview的宽度定义子控件的宽度\n我发现这个lable没设置高度，这是为什呢？有谁可以解释一下吗？他竟然可以自动更新高度。。。";
